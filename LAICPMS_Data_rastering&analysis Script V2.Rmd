...
Sample_element_patterns_list[["Sample_23_10 1 element_patterns_data"]] %>%
  ...
  ggplot() +
  geom_path(aes(x = distance.um, y = Li_Mg_proxy), col = "red") +# does not seem right?? Look through Cuny-Cuirriec et al., 2019 Chem. Geol.
  ...
  geom_label(aes(y = 40, x = 300, label = "Li/Mg proxy - Montagna et al., 2014"),col = "red") +
...
# Are we seeing seawater manipulation in the Li_Ca department because they are deep-water?
# Calcite contribution on the Li/Mg proxy - these corals have a very high calcite concentration, so may need to be corrected

# (log(Li_Mg/5.38))/-0.048
formula = y ~ s(x, bs = "cs")
Sample_element_patterns_list[["Sample_23_10 1 element_patterns_data"]] %>%
  group_by(distance.um) %>% # calculate across all tracks
  summarise(Li_Ca = Li_Ca,
