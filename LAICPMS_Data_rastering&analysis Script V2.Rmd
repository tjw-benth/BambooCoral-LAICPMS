...
for(j in 1:length(Samples)) { # 4 samples
  ...
  for(i in 70:78) { # 9 element ratios
    ...
    Tracks_data <- data.frame(Track1 = Tracks[[1]][[i]],
                              Track2 = Tracks[[2]][[i]],
                              Track3 = Tracks[[3]][[i]],
                              Track4 = Tracks[[4]][[i]],
                              Track5 = Tracks[[5]][[i]]) %>%
...
Rasterpath_viridis <- "~/Library/CloudStorage/OneDrive-UniversityofSouthampton/PhD, INSPIRE DTP/Countries_Chapters/UK [Soton]/Chpt4 Bamboo Coral Analysis/LAICPMS/Data_R_processed/NIST612_Corrected_rasters/Interpolated_Viridis/"
b <- 1
col2 <- viridis(24)
plot.new()
for (b in 1:length(Li_Ca_raster_list)) {
  par(mfrow=c(1,1),
                     ...
                     bty = "n",
                     bg = "transparent")
plot(Li_Ca_raster_list[[b]],col=(col2), interpolate=TRUE, axes = F, cex.axis=0.8, asp = 1.25, alpha = 0.6, legend.args=list(text=" Li/Ca (µmol/l)", side=4, line=3,cex=0.8, las=0), breaks = seq(0,800,by = 5))
...
ggplot(data = Sample_element_patterns_list[["Sample_23_10 1 element_patterns_data"]]) +
  geom_path(aes(x = distance.um, y = mean(Li_Ca, na.rm = T)), col = "grey") +
  geom_path(aes(x = distance.um, y = mean(Mg_Ca, na.rm = T)/1000), col = "orange") # does not seem right??
