...
plot_a_list <- function(master_list_with_plots, no_of_rows, no_of_cols, ylim, xlim, guides) {
...
Conversions <- data.frame(Conversion = c("nmol/mol", "µmol/mol", "mmol/mol"),
...
# This chunk extracts all LAICPMS runs, quality checks, tags and merges into a list of dataframes ####
## all relevant files are csv

ICPMS_csvfilelist <- list.files(path = paste0(here::here("Bamboo Corals 05May22")), 
                             pattern = ".csv", 
                             full.names = T, 
...
# now rename the un-needed dataframes in the large list based off the directory file sample IDs
...
# This chunk separates out the runs into standards and coral samples and processes the data (blank bracketing, analysis bracketing, outlier removal (standards only), standard deviation cut offs, blank corrections) ####
...
Signals_graph_folder <- paste0(here::here("Element_Signals"))
...
# want to reject "bad" data in blanks - use +/- 4sd limit
# determine means and s.d of each element
m.Preblank <- Preblank %>%
  summarise(across(all_of(c(12:21)), ~ mean(., na.rm = T), .names = "{col}_mean"))

sd.Preblank <- Preblank %>%
  summarise(across(all_of(c(12:21)), ~ sd(., na.rm = T), .names = "{col}_sd"))

m.Postblank <- Postblank %>%
  summarise(across(all_of(c(12:21)), ~ mean(., na.rm = T), .names = "{col}_mean"))

sd.Postblank <- Postblank %>%
  summarise(across(all_of(c(12:21)), ~ sd(., na.rm = T), .names = "{col}_sd"))

no.outliers.Preblank <- matrix(NA, ncol = ncol(m.Preblank), nrow = nrow(Preblank))
no.outliers.Postblank <- matrix(NA, ncol = ncol(m.Postblank), nrow = nrow(Postblank))

# ifelse statement to evaluate each data point for each element against specific s.d. of the mean for that element

Elements <- c("Li7", "B11", "Mg24", "P31", "S34", "Ca43", "Mn55", "Sr86", "Ba137", "U238")

for(x in 1:ncol(m.Preblank)) {
  if (Elements[x] == "U238" | Elements[x] == "Ba137" | Elements[x] == "Mn55") {
  R<-2 
} else { if(Elements[x] == "P31") {
  R<-3
} else { if (Elements[x] == "Li7" | Elements[x] == "Sr86") {
  R<-3
} else {
  R<-4
  }
  ...
  for (o in 1:nrow(Preblank)) {
   no.outliers.Preblank[o,x] <- ifelse(Preblank[o,11+x] < m.Preblank[x] + R*sd.Preblank[x] & 
                                         Preblank[o,11+x] > m.Preblank[x] - R*sd.Preblank[x], Preblank[o,11+x], NA)
  }
}

for(x in 1:ncol(m.Postblank)) {
  for (o in 1:nrow(Postblank)) {
   no.outliers.Postblank[o,x] <- ifelse(Postblank[o,11+x] < m.Postblank[x] + R*sd.Postblank[x] & 
                                         Postblank[o,11+x] > m.Postblank[x] - R*sd.Postblank[x], Postblank[o,11+x], NA)
  }
}

colnames(no.outliers.Preblank) <- names(Preblank[12:21])
no.outliers.Preblank <- as_tibble(no.outliers.Preblank)

colnames(no.outliers.Postblank) <- names(Postblank[12:21])
no.outliers.Postblank <- as_tibble(no.outliers.Postblank)

Preblank_V2 <- Runs_612[[j]] %>%
  filter(`Time [Sec]` <= 30.5) %>%
  cbind(., no.outliers.Preblank)

Postblank_V2 <- Runs_612[[j]] %>%
  filter(`Time [Sec]` >= max(`Time [Sec]`, na.rm = T)-45) %>%
  cbind(., no.outliers.Postblank)

Allblanks <- full_join(Preblank_V2, Postblank_V2)
...
# want to reject "bad" data in analysis - use +/- 4sd limit
# determine means and s.d of each element
m.Analysis <- Analysis %>%
  summarise(across(all_of(c(12:21)), ~ mean(., na.rm = T), .names = "{col}_mean"))
...
no.outliers.Analysis <- matrix(NA, ncol = ncol(m.Analysis), nrow = nrow(Analysis))


# ifelse statement to evaluate each data point for each element against specific s.d. of the mean for that element
Elements <- c("Li7", "B11", "Mg24", "P31", "S34", "Ca43", "Mn55", "Sr86", "Ba137", "U238")

for(x in 1:ncol(m.Analysis)) {
  if (Elements[x] == "U238" | Elements[x] == "Ba137" | Elements[x] == "Mn55") {
  R<-2 
} else { if(Elements[x] == "P31") {
  R<-3
  ...
  for (o in 1:nrow(Analysis)) {
   no.outliers.Analysis[o,x] <- ifelse(Analysis[o,11+x] < m.Analysis[x] + R*sd.Analysis[x] & 
                                         Analysis[o,11+x] > m.Analysis[x] - R*sd.Analysis[x], Analysis[o,11+x], NA)
  }
}

colnames(no.outliers.Analysis) <- names(Analysis[12:21])
no.outliers.Analysis <- as_tibble(no.outliers.Analysis)

Analysis_V2 <- Runs_612[[j]] %>%
  filter(`Time [Sec]` >= Pre_blank_cut_off & `Time [Sec]` <= Post_blank_cut_off) %>%
  cbind(., no.outliers.Analysis)

Analysis_full <- full_join(Allblanks, Analysis_V2)
...
rm(Allblanks, Analysis, Analysis_full, Analysis_V2, Analysis_full_blankcorrections_ratios, Blankcorrectios, Blankcorrections_ratios, Data_averages, Li_Ca, m.Analysis, m.Postblank, m.Preblank, meancutoffs, meancutoffs_data, 
   no.outliers.Analysis, no.outliers.Postblank, no.outliers.Preblank, o, Postblank, Post_blank_cut_off, Postblank_V2,
   Pre_blank_cut_off, Preblank, Preblank_V2, sd.Analysis, sd.Postblank, sd.Preblank)

}
plot_a_list(Elementdata_612, 7,2, guides = "collect")
...
# Restrict to first 50 seconds in case there is a jump in the calcium in the middle of the standard (due to artifcats)
Cut_off1 <- max(Delta_Ca_dataframe[which(Delta_Ca_dataframe$Time <= 50),]$Delta_Ca, na.rm = T)
# In case there is a jump in the calcium at the exterior of the coral (due to calcification boundary)
Cut_off2 <- min(Delta_Ca_dataframe[which(Delta_Ca_dataframe$Time >= max(Delta_Ca_dataframe$Time, na.rm = T)/2),]$Delta_Ca, na.rm = T)
Pre_blank_cut_off <- Delta_Ca_dataframe[which(Delta_Ca_dataframe$Delta_Ca == Cut_off1),]$Time
...
Preblank_V2 <- Runs_JCP[[j]] %>%
  filter(`Time [Sec]` <= 30.5) %>%
  cbind(., no.outliers.Preblank)

Postblank_V2 <- Runs_JCP[[j]] %>%
  filter(`Time [Sec]` >= max(`Time [Sec]`, na.rm = T)-45) %>%
  cbind(., no.outliers.Postblank)
...
plot_a_list(Elementdata_JCP, 7,2, guides = "collect")
...
for (a in 2:11) {
  datapoints <- datapoints + geom_line(aes_string(y = names(Runs_DSC[[j]])[a], color =  shQuote(names(Runs_DSC[[j]])[a])))
}
...
Preblank_V2 <- Runs_DSC[[j]] %>%
  filter(`Time [Sec]` <= 30.5) %>%
  cbind(., no.outliers.Preblank)

Postblank_V2 <- Runs_DSC[[j]] %>%
  filter(`Time [Sec]` >= max(`Time [Sec]`, na.rm = T)-45) %>%
  cbind(., no.outliers.Postblank)
...
plot_a_list(Elementdata_DSC, 6,2, guides = "collect")
...
Allblanks <- full_join(Runs_Sample_23_16_1[[j]], Preblank) %>%
  full_join(Postblank)

Analysis <- Runs_Sample_23_16_1[[j]] %>%
  filter(`Time [Sec]` >= Pre_blank_cut_off & `Time [Sec]` <= Post_blank_cut_off) %>% # analysis started at 40 seconds and finished 56 seconds before end of ablation
  mutate(across(all_of(c(2:11)), ~ ., .names = "{col}_analysis"))

Analysis_full <- full_join(Allblanks, Analysis)
...
plot_a_list(Elementdata_Sample_23_16_1, 2,3, guides = "collect")
...
for (a in 2:11) {
  datapoints <- datapoints + geom_line(aes_string(y = names(Runs_Sample_23_16_2[[j]])[a], color =  shQuote(names(Runs_Sample_23_16_2[[j]])[a])))
}
...
Postblank <- Runs_Sample_23_16_2[[j]] %>%
  filter(`Time [Sec]` >= max(`Time [Sec]`, na.rm = T)-50) %>%
  mutate(across(all_of(c(2:11)), ~ ., .names = "{col}_postblank"))

Allblanks <- full_join(Runs_Sample_23_16_2[[j]], Preblank) %>%
  full_join(Postblank)

Analysis <- Runs_Sample_23_16_2[[j]] %>%
  filter(`Time [Sec]` >= Pre_blank_cut_off & `Time [Sec]` <= Post_blank_cut_off) %>% # analysis started at 32 seconds and finished 56 seconds before end of ablation
  mutate(across(all_of(c(2:11)), ~ ., .names = "{col}_analysis"))
...
plot_a_list(Elementdata_Sample_23_16_2, 2,3, guides = "collect")
...
nc<-wes_palette(length(names(Runs_Sample_23_16_3[[j]][2:11])), name="Darjeeling1", type="continuous")
...
Allblanks <- full_join(Runs_Sample_23_16_3[[j]], Preblank) %>%
  full_join(Postblank)

Analysis <- Runs_Sample_23_16_3[[j]] %>%
  filter(`Time [Sec]` >= Pre_blank_cut_off & `Time [Sec]` <= Post_blank_cut_off) %>% # analysis started at 38 seconds and finished 58 seconds before end of ablation
  mutate(across(all_of(c(2:11)), ~ ., .names = "{col}_analysis"))

Analysis_full <- full_join(Allblanks, Analysis)

# Li_datapoints <- ggplot() +
#   geom_point(aes_(Analysis_full$`Time [Sec]`,Analysis_full$Li7)) + 
#   geom_point(aes_(Analysis_full$`Time [Sec]`,Analysis_full$Li7_analysis), col = "red") +
#   annotate("text", x = Inf, y = Inf, hjust = 2, vjust = 2, label = "Analysis", col = "red") +
#   annotate("text", x = -Inf, y = Inf, hjust = -.25, vjust = 2, label = names(Runs_Sample_23_16_3[j]), col = "black")
# 
# assign(paste0(names(Runs_Sample_23_16_3[j]), "_Lithiumdata"), Li_datapoints)
# Lithiumdata_Sample_23_16_3[[paste0(names(Runs_Sample_23_16_3[j]), "_Lithiumdata")]] <- Li_datapoints
...
plot_a_list(Elementdata_Sample_23_16_3, 2,3, guides = "collect")
...
Postblank <- Runs_Sample_23_1_1[[j]] %>%
  filter(`Time [Sec]` >= max(`Time [Sec]`, na.rm = T)-48) %>%
  mutate(across(all_of(c(2:11)), ~ ., .names = "{col}_postblank"))

Allblanks <- full_join(Runs_Sample_23_1_1[[j]], Preblank) %>%
  full_join(Postblank)

Analysis <- Runs_Sample_23_1_1[[j]] %>%
  filter(`Time [Sec]` >= Pre_blank_cut_off & `Time [Sec]` <= Post_blank_cut_off) %>% # analysis started at 41 seconds and finished 58 seconds before end of ablation
  mutate(across(all_of(c(2:11)), ~ ., .names = "{col}_analysis"))
...
# 
# assign(paste0(names(Runs_Sample_23_1_1[j]), "_Lithiumdata"), Li_datapoints)
# Lithiumdata_Sample_23_1_1[[paste0(names(Runs_Sample_23_1_1[j]), "_Lithiumdata")]] <- Li_datapoints
...
plot_a_list(Elementdata_Sample_23_1_1, 2,3, guides = "collect")
...
Allblanks <- full_join(Runs_Sample_23_1_2[[j]], Preblank) %>%
  full_join(Postblank)
...
# 
# assign(paste0(names(Runs_Sample_23_1_2[j]), "_Lithiumdata"), Li_datapoints)
# Lithiumdata_Sample_23_1_2[[paste0(names(Runs_Sample_23_1_2[j]), "_Lithiumdata")]] <- Li_datapoints
...
plot_a_list(Elementdata_Sample_23_1_2, 2,3, guides = "collect")
...
Allblanks <- full_join(Runs_Sample_23_1_3[[j]], Preblank) %>%
  full_join(Postblank)

Analysis <- Runs_Sample_23_1_3[[j]] %>%
  filter(`Time [Sec]` >= Pre_blank_cut_off & `Time [Sec]` <= Post_blank_cut_off) %>% # analysis started at 32 seconds and finished 51 seconds before end of ablation
  mutate(across(all_of(c(2:11)), ~ ., .names = "{col}_analysis"))
...
# 
# assign(paste0(names(Runs_Sample_23_1_3[j]), "_Lithiumdata"), Li_datapoints)
# Lithiumdata_Sample_23_1_3[[paste0(names(Runs_Sample_23_1_3[j]), "_Lithiumdata")]] <- Li_datapoints
...
plot_a_list(Elementdata_Sample_23_1_3, 2,3, guides = "collect")
...
Postblank <- Runs_Sample_23_10_1[[j]] %>%
  filter(`Time [Sec]` >= max(`Time [Sec]`, na.rm = T)-46) %>%
  mutate(across(all_of(c(2:11)), ~ ., .names = "{col}_postblank"))

Allblanks <- full_join(Runs_Sample_23_10_1[[j]], Preblank) %>%
  full_join(Postblank)

Analysis <- Runs_Sample_23_10_1[[j]] %>%
  filter(`Time [Sec]` >= Pre_blank_cut_off & `Time [Sec]` <= Post_blank_cut_off)  %>% # analysis started at 35 seconds and finished 55 seconds before end of ablation
  mutate(across(all_of(c(2:11)), ~ ., .names = "{col}_analysis"))
...
# 
# assign(paste0(names(Runs_Sample_23_10_1[j]), "_Lithiumdata"), Li_datapoints)
# Lithiumdata_Sample_23_10_1[[paste0(names(Runs_Sample_23_10_1[j]), "_Lithiumdata")]] <- Li_datapoints
...
plot_a_list(Elementdata_Sample_23_10_1, 2,3, guides = "collect")
...
Allblanks <- full_join(Runs_Sample_23_10_2[[j]], Preblank) %>%
  full_join(Postblank)

Analysis <- Runs_Sample_23_10_2[[j]] %>%
  filter(`Time [Sec]` >= Pre_blank_cut_off & `Time [Sec]` <= Post_blank_cut_off) %>% # analysis started at 44 seconds and finished 55 seconds before end of ablation
  mutate(across(all_of(c(2:11)), ~ ., .names = "{col}_analysis"))
...
# 
# assign(paste0(names(Runs_Sample_23_10_2[j]), "_Lithiumdata"), Li_datapoints)
# Lithiumdata_Sample_23_10_2[[paste0(names(Runs_Sample_23_10_2[j]), "_Lithiumdata")]] <- Li_datapoints
...
plot_a_list(Elementdata_Sample_23_10_2, 2,3, guides = "collect")
...
Allblanks <- full_join(Runs_Sample_23_10_3[[j]], Preblank) %>%
  full_join(Postblank)

Analysis <- Runs_Sample_23_10_3[[j]] %>%
  filter(`Time [Sec]` >= Pre_blank_cut_off & `Time [Sec]` <= Post_blank_cut_off) %>% # analysis started at 42 seconds and finished 55 seconds before end of ablation
  mutate(across(all_of(c(2:11)), ~ ., .names = "{col}_analysis"))
...
# 
# assign(paste0(names(Runs_Sample_23_10_3[j]), "_Lithiumdata"), Li_datapoints)
# Lithiumdata_Sample_23_10_3[[paste0(names(Runs_Sample_23_10_3[j]), "_Lithiumdata")]] <- Li_datapoints
...
plot_a_list(Elementdata_Sample_23_10_3, 2,3, guides = "collect")
...
Allblanks <- full_join(Runs_Sample_23_6_1[[j]], Preblank) %>%
  full_join(Postblank)

Analysis <- Runs_Sample_23_6_1[[j]] %>%
  filter(`Time [Sec]` >= Pre_blank_cut_off & `Time [Sec]` <= Post_blank_cut_off) %>% # analysis started at 47 seconds and finished 80 seconds before end of ablation
  mutate(across(all_of(c(2:11)), ~ ., .names = "{col}_analysis"))
...
# 
# assign(paste0(names(Runs_Sample_23_6_1[j]), "_Lithiumdata"), Li_datapoints)
# Lithiumdata_Sample_23_6_1[[paste0(names(Runs_Sample_23_6_1[j]), "_Lithiumdata")]] <- Li_datapoints
...
plot_a_list(Elementdata_Sample_23_6_1, 2,3, guides = "collect")
...
Allblanks <- full_join(Runs_Sample_23_6_2[[j]], Preblank) %>%
  full_join(Postblank)

Analysis <- Runs_Sample_23_6_2[[j]] %>%
  filter(`Time [Sec]` >= Pre_blank_cut_off & `Time [Sec]` <= Post_blank_cut_off) %>% # analysis started at 42 seconds and finished 70 seconds before end of ablation
  mutate(across(all_of(c(2:11)), ~ ., .names = "{col}_analysis"))
...
# 
# assign(paste0(names(Runs_Sample_23_6_2[j]), "_Lithiumdata"), Li_datapoints)
# Lithiumdata_Sample_23_6_2[[paste0(names(Runs_Sample_23_6_2[j]), "_Lithiumdata")]] <- Li_datapoints
...
plot_a_list(Elementdata_Sample_23_6_2, 2,3, guides = "collect")
...
Allblanks <- full_join(Runs_Sample_23_6_3[[j]], Preblank) %>%
  full_join(Postblank)
...
# 
# assign(paste0(names(Runs_Sample_23_6_3[j]), "_Lithiumdata"), Li_datapoints)
# Lithiumdata_Sample_23_6_3[[paste0(names(Runs_Sample_23_6_3[j]), "_Lithiumdata")]] <- Li_datapoints
...
plot_a_list(Elementdata_Sample_23_6_3, 2,3, guides = "collect")
...
JCP_Corrected_path <- paste0(here::here("Data_R_processed", "JCP_Corrected"), "/")

# remember need to keep all ratios to same unit (µmol/mol)
...
rm(JCP_Corrected_Sample_23_16_1, JCP_Corrected_Sample_23_16_2, JCP_Corrected_Sample_23_16_3)
...
rm(JCP_Corrected_Sample_23_1_1, JCP_Corrected_Sample_23_1_2, JCP_Corrected_Sample_23_1_3)
...
rm(JCP_Corrected_Sample_23_10_1, JCP_Corrected_Sample_23_10_2, JCP_Corrected_Sample_23_10_3)
...
rm(JCP_Corrected_Sample_23_6_1, JCP_Corrected_Sample_23_6_2, JCP_Corrected_Sample_23_6_3)
...
rm(JCP_Corrected_Sample_23_16, JCP_Corrected_Sample_23_1, JCP_Corrected_Sample_23_10, JCP_Corrected_Sample_23_6)
...
for (i in 1:length(JCP_Corrected_Samples)) {
  for (j in 1:length(JCP_Corrected_Samples[[i]])) {
    for (x in 1:length(JCP_Corrected_Samples[[i]][[j]])) {
      filename = paste0(JCP_Corrected_path, names(JCP_Corrected_Samples[[i]][[j]])[[x]], ".csv")
...
NIST612_Corrected_path <- paste0(here::here("Data_R_processed", "NIST612_Corrected"), "/")
...
rm(NIST612_Corrected_Sample_23_16_1, NIST612_Corrected_Sample_23_16_2, NIST612_Corrected_Sample_23_16_3)
...
rm(NIST612_Corrected_Sample_23_1_1, NIST612_Corrected_Sample_23_1_2, NIST612_Corrected_Sample_23_1_3)
...
rm(NIST612_Corrected_Sample_23_10_1, NIST612_Corrected_Sample_23_10_2, NIST612_Corrected_Sample_23_10_3)
...
rm(NIST612_Corrected_Sample_23_6_1, NIST612_Corrected_Sample_23_6_2, NIST612_Corrected_Sample_23_6_3)
...
rm(NIST612_Corrected_Sample_23_16, NIST612_Corrected_Sample_23_1, NIST612_Corrected_Sample_23_10, NIST612_Corrected_Sample_23_6)
...
for (i in 1:length(NIST612_Corrected_Samples)) {
  for (j in 1:length(NIST612_Corrected_Samples[[i]])) {
    for (x in 1:length(NIST612_Corrected_Samples[[i]][[j]])) {
      filename = paste0(NIST612_Corrected_path,names(NIST612_Corrected_Samples[[i]][[j]])[[x]], ".csv")
