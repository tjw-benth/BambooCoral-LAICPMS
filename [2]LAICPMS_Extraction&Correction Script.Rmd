...
# ifelse statement to evaulate each data point for each element against 4 s.d. of the mean for that element
for(x in 1:ncol(m.Preblank)) {
  for (o in 1:nrow(Preblank)) {
   no.outliers.Preblank[o,x] <- ifelse(Preblank[o,11+x] < m.Preblank[x] + 4*sd.Preblank[x] & 
                                         Preblank[o,11+x] > m.Preblank[x] - 4*sd.Preblank[x], Preblank[o,11+x], NA)
  }
}

for(x in 1:ncol(m.Postblank)) {
  for (o in 1:nrow(Postblank)) {
   no.outliers.Postblank[o,x] <- ifelse(Postblank[o,11+x] < m.Postblank[x] + 4*sd.Postblank[x] & 
                                         Postblank[o,11+x] > m.Postblank[x] - 4*sd.Postblank[x], Postblank[o,11+x], NA)
  }
}
...
for(x in 1:ncol(m.Analysis)) {
  for (o in 1:nrow(Analysis)) {
   no.outliers.Analysis[o,x] <- ifelse(Analysis[o,11+x] < m.Analysis[x] + 4*sd.Analysis[x] & 
                                         Analysis[o,11+x] > m.Analysis[x] - 4*sd.Analysis[x], Analysis[o,11+x], NA)
  }
}
